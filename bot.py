...
PUBLISH_HOURS = "5,7,9,11,13,15,17,19"

# ─── Поисковые запросы ────────────────────────────────────────────────────────

UNSPLASH_QUERIES = [
    "seagull", "seagull flying", "seagull beach", "seagull ocean",
    "herring gull", "laughing gull", "seagull port", "seagull cliff",
    "gull bird", "seagull sky", "gull", 
]

PEXELS_QUERIES = [
    "seagull", "seagull flying", "seagull beach", "herring gull",
    "gull bird", "seagull ocean", "seagull cliff", "чайка", "gull",
]

# Сколько страниц листать (каждая страница = 5 фото)
PAGES_TO_FETCH = 3
            ...
            for page in range(1, PAGES_TO_FETCH + 1):
                try:
                    async with session.get(
                        ...
                        params={"query": query, "per_page": 5, "page": page, "order_by": "latest"},
                        headers=headers
                    ) as resp:
                        if resp.status == 200:
                    ...
                    print(f"Ошибка Unsplash ({query} p{page}): {e}")
    return results
        ...
        for query in PEXELS_QUERIES:
            for page in range(1, PAGES_TO_FETCH + 1):
                try:
                    ...
                    print(f"Ошибка Pexels ({query} p{page}): {e}")
    return results
...
async def button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "s":
        ...
        prompt = "✏️ <b>Напиши описание для публикации в канал</b>\nОтправь следующим сообщением."
        if post.get("link"):
...
async def cmd_clearqueue(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        return
    save_json(QUEUE_FILE, [])
    await update.message.reply_text("🗑️ Очередь очищена!")
...
def main():
    ...
    scheduler.add_job(check_feeds, "interval", hours=8, args=[app])
